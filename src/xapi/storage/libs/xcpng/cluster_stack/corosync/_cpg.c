...
static int py_cpg_init_callbacks() {
    py_cpg_callbacks = calloc(py_cpg_start_callbacks, sizeof(py_cpg_callbacks_t));
...
static int py_cpg_add_callbacks(cpg_handle_t handle, PyObject *callbacks) {
    if (py_cpg_nr_callbacks == py_cpg_alloc_callbacks) {
        py_cpg_callbacks = realloc(py_cpg_callbacks, py_cpg_alloc_callbacks * 2 * sizeof(py_cpg_callbacks_t));
        if (py_cpg_callbacks == NULL) {
            PyErr_NoMemory();
            return -1;
        }
...
static void py_cpg_remove_callbacks(cpg_handle_t handle) {
    int i;

    for (i=0; i<py_cpg_nr_callbacks; i++) {
        if (py_cpg_callbacks[i].handle == handle) {
            memmove(py_cpg_callbacks + i, py_cpg_callbacks + i + 1, (py_cpg_nr_callbacks - i) * sizeof(py_cpg_callbacks_t));
            py_cpg_nr_callbacks -= 1;
            break;
        }
    }
...
static PyObject* py_cpg_find_callbacks(cpg_handle_t handle) {
    int i;
...
static void py_cpg_deliver_fn(cpg_handle_t handle, struct cpg_name *group_name,
    ...
    Pret = PyObject_CallMethodObjArgs(Pcallbacks, Pmethod, Pname, Paddr, Pmessage, NULL);
...
static void py_cpg_confchg_fn(cpg_handle_t handle, struct cpg_name *group_name,
    ...
    PyObject *Pcallbacks, *Pname, *Pmembers, *Paddr, *Pleft, *Pjoined, *Pmethod, *Pret;
    ...
    Pret = PyObject_CallMethodObjArgs(Pcallbacks, Pmethod, Pname, Pmembers, Pleft, Pjoined, NULL);
...
static PyObject* py_cpg_initialize(PyObject *self, PyObject *args) {
    int ret;
...
static PyObject* py_cpg_finalize(PyObject *self, PyObject *args) {
    int ret;
    cpg_handle_t handle;
...
static PyObject* py_cpg_join(PyObject *self, PyObject *args) {
    char *name;
    int length, ret;
    cpg_handle_t handle;
    ...
    if (length > CPG_MAX_NAME_LENGTH) {
        PyErr_Format(py_cpg_error, "Group name too long (%d, max length = %d)", length, CPG_MAX_NAME_LENGTH);
        return NULL;
    }
...
static PyObject* py_cpg_leave(PyObject *self, PyObject *args) {
    char *name;
    int length, ret;
    cpg_handle_t handle;
    struct cpg_name group_name;
...
static PyObject* py_cpg_dispatch(PyObject *self, PyObject *args) {
    int ret;
    cpg_handle_t handle;
...
static PyObject* py_cpg_fd_get(PyObject *self, PyObject *args) {
    int fd, ret;
    cpg_handle_t handle;
    PyObject *Phandle;
...
static PyObject* py_cpg_mcast_joined(PyObject *self, PyObject *args) {
    int ret;
    void *data;
    size_t length;
    cpg_handle_t handle;
...
static PyObject* py_cpg_local_get(PyObject *self, PyObject *args) {
    cpg_handle_t handle;
    unsigned int nodeid;
    int ret;
    PyObject *Presult;
    ...
    RETURN_ON_ERROR(ret, "cpg_local_get");

    Presult = PyInt_FromLong(nodeid);
    return Presult;
}

static PyObject* py_cpg_membership_get(PyObject *self, PyObject *args) {
    cpg_handle_t handle;
    struct cpg_name group_name;
...
static py_cpg_constant_t py_cpg_constants[] = {
    ...
    { "DISPATCH_ONE", CS_DISPATCH_ONE },
    { "DISPATCH_ALL", CS_DISPATCH_ALL },
    { "DISPATCH_BLOCKING", CS_DISPATCH_BLOCKING },
...
static PyMethodDef py_cpg_methods[] =  {
    ...
    { "local_get", (PyCFunction) py_cpg_local_get, METH_VARARGS },
...
void init_cpg(void) {
    ...
    for (i=0; py_cpg_constants[i].name != NULL; i++) {
        PyDict_SetItemString(Pdict, py_cpg_constants[i].name, PyInt_FromLong(py_cpg_constants[i].value));
    }
}
